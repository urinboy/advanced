<?php
    return [

        // Site mine and desscription
        "Masofaviy Ta'lim" => "Distance Learning",
        "Qoraqalpoq davlat universiteti" =>"Karakalpak State University",
        "/logos/karsu_moodle-UZ.png" => "/logos/karsu_moodle-EN.png",
        "Platformamizga hush kelibsiz!" => "Welcome to our platform!",
            "Agar savollaringiz bo'lsa, biz bilan bog'lanish uchun quyidagi shaklni to'ldiring. Rahmat." => "If you have any questions, please fill out the form below to contact us. Thank you.",
        "Biz bilan bog'langaningiz uchun tashakkur. Sizga imkon qadar tezroq javob beramiz." => "Thank you for contacting us. We will reply you as soon as possible.",

        // Kontakt formasida
        "To'liq ism" => "Full Name",
        "Elektron pochta" => "Email",
        "Mavzu" => "Subject",
        "To'liq matn" => "Body",
        "Tasdiq kodi" => "Verification Code",
        "Yuborish" => "Submit",

        // Kirish formasida
        "Login" => "Login",
        "Parol" => "Password",
        "Eslab qolinsinmi" => "Remember me",

        // Navbar translators
        "Bosh sahifa" => "Home",
        "Biz haqimizda" => "About",
        "Yangiliklar" => "News",
        "Bog'lanish" => "Contact",

        // kirish chiqish ro'yxatga olish
        "Kirish" => "Login",
        "Ro'yxatdan o'tish" => "Register",
        "Profil" => "Profile",
        "Chiqish (" => "Log out (",
        "Roʻyxatdan oʻtganingiz uchun tashakkur. E-pochtani tasdiqlash uchun pochta qutingizni tekshiring." => "Thank you for registration. Please check your inbox for verification email.",

        // Errors
        "Yuqoridagi xato veb-server sizning so'rovingizni ko'rib chiqayotganda sodir bo'ldi." => "The above error occurred while the web server was considering your request.",
        "Agar bu server xatoligi deb o'ylasangiz, biz bilan bog'laning. Rahmat." => "If you think this is the server error, contact us. Thank you.",

    ];